...
def read_xer(xer_file):
    ...
    with open(xer_file, 'r', encoding='latin1') as xer:
        ...
        for line in xer:
            ...
            table_columns, table_data = [], []            # %F = fields
            if split_line[0] == '%F':
                table_columns = split_line[1:]            # %R = record
            # ERRORS WITH SOME TABLES?
            if split_line[0] == '%R':
                table_data.append(split_line[1:])            # %E = end
            if split_line[0] == '%E' and table_name is not None:
                try:
                    tables[table_name] = data_to_table(table_columns, table_data)
                except:
                    pass
    return tables